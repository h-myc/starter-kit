...
const PATTERNS = [
    { name: 'Generic Secret', regex: /(api_key|secret|token)[\s=:"']*([A-Za-z0-9+/]{20,})/i },
...
try {
    ...
    stagedFiles.forEach(file => {
        ...
        if (file.includes('check-secrets.js') || file.includes('package-lock.json')) return;

        const content = fs.readFileSync(file, 'utf-8');
        PATTERNS.forEach(pattern => {
            if (pattern.regex.test(content)) {
                console.error(`[Security] Potential secret found in ${file}: ${pattern.name}`);
                failed = true;
            }
        });
    });
    ...
    if (failed) {
        console.error('[Security] Commit blocked. Remove secrets or bypass with --no-verify if false positive.');
        process.exit(1);
    } else {
